...
Acá mostrare una adaptación de uno de los primeros blogs con contenido de R que revise escrito por **Sean Kross**[^note]. 
...
replicas %>%
  mutate(prom_real =
        case_when(lim_inf < 0.4994207 & 0.4994207<lim_sup ~ 1, TRUE ~ 0)) %>% summarise(mean(prom_real))
```
...
[^note]: Basado en **A Short Intro to Confidence Intervals** por [Sean Kross](http://seankross.com/).
